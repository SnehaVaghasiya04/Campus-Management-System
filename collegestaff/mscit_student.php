...
$result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM cstudents WHERE course='MSC IT'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    if ($date !== $current_date) {
        echo "<script>alert('You can only mark attendance for today ($formatted_date)!'); window.location.href='mscit_student.php';</script>";
        exit();
    }
    ...
    if (mysqli_num_rows($checkAttendance) > 0) {
        echo "<script>alert('Attendance for today ($formatted_date) has already been marked!'); window.location.href='mscit_student.php';</script>";
        exit();
    }

    if (!isset($_POST['attendance']) || empty($_POST['attendance'])) {
        echo "<script>alert('No students selected. Please mark attendance.'); window.location.href='mscit_student.php';</script>";
        exit();
    }
    ...
    echo "<script>alert('Attendance marked successfully! Report Generated.'); window.location.href='mscit_student.php';</script>";
}
?>
...
<div class="container mt-4">
    <div class="container1">
        <fieldset>
            <legend><h1>Mark MSC IT Attendance</h1></legend>
            ...
            <form method="post">
                ...
                <table>
                    ...
                    <?php
                    $students = mysqli_query($conn, "SELECT student_id, name FROM cstudents WHERE course='MSC IT' LIMIT $start, $limit");
